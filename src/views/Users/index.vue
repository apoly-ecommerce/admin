  ...
  <section class="PageUser">
    ...
    <page-table-content :tableName="tableName">
      <template v-slot:tools>
        ...
        <router-link class="Table__tools-item" :to="{ name: 'add-user' }">Thêm mới</router-link>
      </template>
      ...
      <template v-slot:main-content>
        <section class="TableBox_Content">
          ...
          >
            ...
            <el-table-column label="Role" prop="role" width="150" sortable>
              <template slot-scope="{row}">
                <el-tag v-if="row.role" size="mini" effect="dark">{{ row.role.name }}</el-tag>
              </template>
            </el-table-column>
...
export default {
  ...
  methods: {
    ...mapActions({
        'fetchListUserByPaginate': 'user/fetchListUserByPaginate',
        'trashUser': 'user/trashUser',
        'massTrashUser': 'user/massTrashUser',
        'fetchListUserTrashedByPaginate': 'user/fetchListUserTrashedByPaginate',
        'destroyUser': 'user/destroyUser',
        'massDestroyUser': 'user/massDestroyUser',
        'restoreUser': 'user/restoreUser',
        'massRestoreUser': 'user/massRestoreUser',
    ...
    getList() {
      ...
      this.fetchListUserByPaginate(this.listQuery).then(res => {
    ...
    getListTrashed() {
      this.listLoading = true;
      this.fetchListUserTrashedByPaginate(this.listQuery).then(res => {
        this.listLoading  = false;
    ...
    handleMassDestroy() {
      ...
      this.$confirm('Xác nhận xóa vĩnh viễn danh sách này ?', 'Xác nhận', {
        ...
        this.massDestroyUser(ids).then(res => {
          this.$message({
            type: 'success',
            message: res.success
          });
    ...
    handleMassTrash() {
      ...
      this.$confirm('Xác nhận chuyển danh sách này vào thùng rác ?', 'Xác nhận', {
        ...
        this.massTrashUser(ids).then(res => {
          this.$message({
            type: 'success',
            message: res.success
          });
    ...
    handleMassRestore() {
      ...
      this.$confirm('Xác nhận khôi phục danh sách này ?', 'Xác nhận', {
        ...
        this.massRestoreUser(ids).then(res => {
          this.$message({
            type: 'success',
            message: res.success
          });
