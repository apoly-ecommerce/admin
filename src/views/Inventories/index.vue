  ...
  <section class="PageInventory">
    ...
    <page-table-content :tableName="tableName">
      <template v-slot:tools>
        ...
        <router-link class="Table__tools-item" :to="{ name: 'search-inventory' }">Thêm mới</router-link>
      </template>
      ...
      <template v-slot:main-content>
        <section class="TableBox_Content">
          ...
          >
            ...
            <el-table-column label="Image" prop="image" width="100">
              <template slot-scope="{row}">
                <div class="TableThumb_ThumbImage">
                  <img :src="row.product.image" alt="" class="image medium">
                </div>
              </template>
            </el-table-column>

            <el-table-column label="SKU" prop="sku" width="100" sortable>
              <template slot-scope="{row}">
            ...
            <el-table-column label="Title" prop="title" width="320" sortable>
              <template slot-scope="{row}">
                <div class="line-txt-top">{{ row.title }}</div>
              </template>
            </el-table-column>

            <el-table-column label="Condition" prop="condition" width="150" sortable>
              <template slot-scope="{row}">
                <div class="line-txt-top">{{ row.condition }}</div>
              </template>
            </el-table-column>

            <el-table-column label="Price" prop="price" width="150" sortable>
              <template slot-scope="{row}">
                <div class="line-txt-top">
                  <template v-if="row.offer_price && checkOfferEndFinished(row.offer_end)">
                    <small>{{ formatCurrency(row.sale_price) }}</small>
                    <div>
                      <span>{{ formatCurrency(row.offer_price) }}</span>
                      <el-tooltip :content="tooltipForOfferPrice(row.offer_start, row.offer_end)" placement="top">
                        <i class="el-icon-info" />
                      </el-tooltip>
                    </div>
                  </template>
                  <div v-else>{{ formatCurrency(row.sale_price) }}</div>
                </div>
              </template>
            </el-table-column>

            <el-table-column label="Quantity" prop="stock_quantity" width="150" sortable>
              <template slot-scope="{row}">
                <div class="line-txt-top">{{ row.stock_quantity }}</div>
              </template>
            </el-table-column>
...
export default {
  ...
  computed: {
    ...mapGetters({
      'tableData': 'inventory/getListInventory',
    }),
    key() {
  ...
  methods: {
    ...mapActions({
        'fetchListInventoryByPaginate': 'inventory/fetchListInventoryByPaginate',
        'trashInventory': 'inventory/trashInventory',
        'massTrashInventory': 'inventory/massTrashInventory',
        'fetchListInventoryTrashedByPaginate': 'inventory/fetchListInventoryTrashedByPaginate',
        'destroyInventory': 'inventory/destroyInventory',
        'massDestroyInventory': 'inventory/massDestroyInventory',
        'restoreInventory': 'inventory/restoreInventory',
        'massRestoreInventory': 'inventory/massRestoreInventory',
        'emptyTrashInventory': 'inventory/emptyTrashInventory'
    }),
    formatTime(time) {
    ...
    getList() {
      ...
      this.fetchListInventoryByPaginate(this.listQuery).then(res => {
        this.listLoading  = false;
        this.isTabTrashed = false;
        this.totalRow = res.total;
        this.removeStatusForm();
      }).catch(error => {
        this.listLoading = false;
    ...
    getListTrashed() {
      this.listLoading = true;
      this.fetchListInventoryTrashedByPaginate(this.listQuery).then(res => {
        this.listLoading  = false;
    ...
    handleRestore(id) {
      this.restoreInventory(id).then(res => {
        this.$message({
          type: 'success',
          message: res.success
        });
    ...
    handleTrash(id) {
      this.$confirm('Xác nhận chuyển vào thùng rác ?', 'Xác nhận', {
        ...
        this.trashInventory(id).then(res => {
          this.$message({
            type: 'success',
            message: res.success
          });
    ...
    handleDestroy(id) {
      this.$confirm('Xác nhận xóa vĩnh viễn khách hàng này ?', 'Xác nhận', {
        ...
        this.destroyInventory(id).then(res => {
          this.$message({
            type: 'success',
            message: res.success
          });
    ...
    handleMassDestroy() {
      ...
      this.$confirm('Xác nhận xóa vĩnh viễn danh sách này ?', 'Xác nhận', {
        ...
        this.massDestroyInventory(ids).then(res => {
          this.$message({
            type: 'success',
            message: res.success
          });
    ...
    handleMassTrash() {
      ...
      this.$confirm('Xác nhận chuyển danh sách này vào thùng rác ?', 'Xác nhận', {
        ...
        this.massTrashInventory(ids).then(res => {
          this.$message({
            type: 'success',
            message: res.success
          });
    ...
    handleEmptyTrash() {
      this.$confirm('Xác nhận xóa vĩnh viễn toàn bộ dữ liệu trong thùng rác ?', 'Xác nhận', {
        ...
        this.emptyTrashInventory().then(res => {
          this.$message({
            type: 'success',
            message: res.success
          });
    ...
    handleMassRestore() {
      ...
      this.$confirm('Xác nhận khôi phục danh sách này ?', 'Xác nhận', {
        ...
        this.massRestoreInventory(ids).then(res => {
          this.$message({
            type: 'success',
            message: res.success
          });
    ...
    tooltipForOfferPrice(offer_start, offer_end) {
      moment.locale('vi');
      offer_start = moment(offer_start).format('DD-MM-YYYY HH:mm');
      offer_end = moment(offer_end).format('DD-MM-YYYY HH:mm');
      return `Khuyến mãi từ ${offer_start} Đến hết ${offer_end}`;
    },
    checkOfferEndFinished(offer_end) {
      moment.locale('vi');
      return moment(offer_end).unix() > moment().unix();
    }
  }
}
</script>
